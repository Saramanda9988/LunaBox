...
`update-server/` 是独立 Cloudflare Worker 项目。R2 保存版本资产，D1 保存下载请求和客户端更新事件。部署与对象布局见 `update-server/README.md`。

发布任务需要配置以下 GitHub Variables：

| 名称 | 用途 |
|------|------|
| `UPDATE_PUBLIC_BASE_URL` | Worker 公网地址 |
| `UPDATE_S3_ENDPOINT` | S3 兼容 API 地址 |
| `UPDATE_S3_BUCKET` | 更新资产 bucket |
| `UPDATE_S3_REGION` | S3 region，R2 使用 `auto` |
