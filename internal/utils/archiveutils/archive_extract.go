...
func ExtractArchive(source, target string) (extracted bool, err error) {
	...
	sevenZipExtracted, sevenZipErr := extractArchiveWithBundled7z(source, target)
	...
	if err != nil {
		if isRecoverableExtractPathError(err) {
			if sevenZipErr != nil {
				return false, fmt.Errorf("extract archive entries: bundled7z=%v; xtractr=%w", sevenZipErr, err)
			}
			return false, fmt.Errorf("extract archive entries: %w", err)
		}
		if sevenZipErr != nil {
			return true, fmt.Errorf("extract archive entries: bundled7z=%v; xtractr=%w", sevenZipErr, err)
		}
